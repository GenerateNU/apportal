...
func registerHandlers(api huma.API, st *store.Store, storageClient *storage.Client) {
	...
	(&assignmentPlannerHandler{store: st}).register(api)
	...
	(&reviewQuestionAverageHandler{store: st}).register(api)
	(&interviewAssignmentHandler{store: st}).register(api)
	(&interviewAssignmentPlannerHandler{store: st}).register(api)
	...
	(&decisionHandler{store: st}).register(api)
}
