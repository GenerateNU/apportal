...
func (h *selectionHandler) register(api huma.API) {
	huma.Register(api, huma.Operation{
		...
		Description: "Reviewer only; upserts the calling TL's selection of this application.",
	...
	huma.Register(api, huma.Operation{
		...
		Summary:     "List a cycle's TL selections",
		Description: "Optional ?tl_nuid= filters to one TL.",
		Tags:        []string{"Selections"},
...
type ListSelectionsInput struct {
	...
	TLNUID string `query:"tl_nuid" doc:"Optional TL filter"`
}
...
func (h *selectionHandler) list(ctx context.Context, in *ListSelectionsInput) (*SelectionsOutput, error) {
	...
	items, err := h.store.ListSelections(ctx, in.ID, in.TLNUID)
	if err != nil {
		return nil, storeErr(err)
	}
